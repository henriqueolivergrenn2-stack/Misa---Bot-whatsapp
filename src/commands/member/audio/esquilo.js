/**
 * Comando: esquilo
 * Deixa o áudio com voz de esquilo e envia como mensagem de voz.
 */
...
export default {
  name: "esquilo",
  description: "Deixa o áudio com voz de esquilo e envia como mensagem de voz!",
  commands: ["esquilo"],
  usage: `${PREFIX}esquilo (responda um áudio)`,
  ...
  handle: async ({
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        exec(
          `ffmpeg -i "${inputPath}" -filter:a "atempo=0.7,asetrate=65100" -c:a libopus -b:a 64k -vn -f ogg -y "${outputPath}"`,
          (err, stdout, stderr) => {
            try { fs.unlinkSync(inputPath); } catch {}
            if (err) {
              console.error("[esquilo ffmpeg]", stderr);
