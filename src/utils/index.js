...
export function extractDataFromMessage(webMessage) {
  ...
  // Fix: fallback para remoteJid quando participant estiver vazio (chat privado)
  const userLid =
    webMessage?.key?.participant?.replace(/:[0-9]+/g, "") ||
    webMessage?.key?.remoteJid?.replace(/:[0-9]+/g, "") ||
    null;
