...
export default {
    name: 'audiorapido',
    description: 'Deixa o áudio mais rápido',
    category: 'efeitos',
    aliases: ['fast'],
    async execute({ columbina, from, info, prefix, reply, reagir, getFileBuffer }) {
        ...
        try {
            ...
            await execPromise(
                `ffmpeg -i "${entrada}" -filter:a "atempo=0.9,asetrate=95100" -c:a libopus -b:a 64k -vn -f ogg -y "${saida}"`,
                { timeout: 60000 }
            );
