...
export default {
    name: 'vozmenino',
    description: 'Deixa o áudio com voz de menino/adolescente',
    category: 'efeitos',
    aliases: ['adolesc'],
    async execute({ columbina, from, info, prefix, reply, reagir, getFileBuffer }) {
        ...
        try {
            ...
            await execPromise(
                `ffmpeg -i "${entrada}" -filter:a "atempo=1.06,asetrate=44100*1.25" -c:a libopus -b:a 64k -vn -f ogg -y "${saida}"`,
                { timeout: 60000 }
            );
            ...
            RedLog(`[VozMenino] Erro ao processar o áudio: ${err.message}\n${err.stack}`);
            await reagir('❌');
