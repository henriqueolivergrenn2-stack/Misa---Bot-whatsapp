...
export default {
    name: 'audiolento',
    description: 'Deixa o áudio mais lento',
    category: 'efeitos',
    aliases: ['slow'],
    async execute({ columbina, from, info, prefix, reply, reagir, getFileBuffer }) {
        ...
        try {
            ...
            await execPromise(
                `ffmpeg -i "${entrada}" -filter:a "atempo=0.85" -c:a libopus -b:a 64k -vn -f ogg -y "${saida}"`,
                { timeout: 60000 }
            );
            ...
            RedLog(`[AudioLento] Erro ao processar o áudio: ${err.message}\n${err.stack}`);
            await reagir('❌');
